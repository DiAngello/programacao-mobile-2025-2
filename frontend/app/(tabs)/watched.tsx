...
export default function WatchedPage() {
  ...
  const handleRemove = (movieId: string) => {
    console.log(`Remover filme ${movieId} da lista de assistidos.`);
  };
  ...
  const renderItem = ({ item }: { item: Movie }) => (
    <TouchableOpacity onPress={() => handleMoviePress(item)} style={styles.movieItem}>
      ...
      <View style={styles.details}>
        ...
        <Text style={styles.rating}>Sua nota: 5/5 ⭐</Text> 
      </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.searchBar}>
        ...
        <TextInput
          placeholder="Pesquisar nos seus filmes assistidos..."
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.emptyText}>
              {searchText ? 'Nenhum filme encontrado.' : 'Sua lista de assistidos está vazia.'}
            </Text>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  rating: { color: COLORS.textSecondary, fontSize: 14, marginTop: 5 },
